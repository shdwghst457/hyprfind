...
> Finder-quality file manager for Hyprland/Linux, built with PyQt6.
> List view first; column/two-pane views deferred to post-MVP.

## Implementation checklist

- [ ] **scaffold** — Create `pyproject.toml`, package layout, README with Mac + CachyOS install/run instructions, dark QSS, `python -m hyprfind` entrypoint
- [ ] **list-view-shell** — MainWindow with QTreeView + QFileSystemModel list columns, toolbar (back/forward/refresh), path bar, basic dark theme
- [ ] **sidebar-nav** — Sidebar favorites + MountService volumes, HistoryStack back/forward, keyboard shortcuts (Enter, Backspace, arrows, type-ahead)
- [ ] **refresh-service** — DirectoryRefreshService: inotify for local paths, polling for SMB/NFS/GVFS; HyprFileSystemModel.refresh_directory(); refresh button + status indicator
- [ ] **preview** — Spacebar Quick Look overlay for images, text, PDF/metadata fallback
- [ ] **linux-polish** — hyprfind.desktop, bookmarks persistence, context menu, tests for mounts/poll; CachyOS SMB validation checklist
...
**Alternative:** Open this plan in Cursor on Cachy and build entirely there — same architecture, faster SMB feedback, slower Mac-side iteration.
...
```python
# Pseudocode — hyprfind/core/refresh.py
...
`MountService` reads `/proc/mounts` (stdlib, no extra dep) and maps longest-prefix mount for any path. Also scan `~/.gvfs` / `/run/user/$UID/gvfs/` for GVFS SMB URLs.
...
- `QApplication` with Fusion style + dark `QPalette` + `hyprfind/ui/styles/dark.qss`
